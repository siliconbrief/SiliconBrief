...
const links = [
  ...
  { title: "DJI Mic Mini 2", href: "https://www.dji.com/uk/mobile/mic-mini-2", image: "logos/DJI Mic Mini 2.png?v=1", initials: "DJ" },
  ...
  .map((item) => {
    ...
    return `
      <a class="link-card" href="${item.href}"${target} data-pinned="${Boolean(item.pinned)}">
        <span class="logo-slot" data-kind="${item.shape || "square"}" data-image-fit="${item.imageFit || "contain"}">${logo}</span>
